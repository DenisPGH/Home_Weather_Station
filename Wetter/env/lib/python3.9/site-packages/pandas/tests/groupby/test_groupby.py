...
def test_len():
    ...
    grouped = df.groupby([lambda x: x.year, lambda x: x.month, lambda x: x.searched_index])
...
def test_groupby_multiple_key():
    ...
    grouped = df.T.groupby(
        [lambda x: x.year, lambda x: x.month, lambda x: x.searched_index], axis=1
    )
...
def test_groupby_reindex_inside_function():
    ...
    def agg_before(func, fix=False):
        ...
        def _func(data):
            d = data.loc[data.index.map(lambda x: x.time_ < 11)].dropna()
            if fix:
                data[data.index[0]]
            if len(d) == 0:
                return None
    ...
    grouped = df.groupby(lambda x: datetime(x.year, x.month, x.searched_index))
