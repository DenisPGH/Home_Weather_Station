...
cdef _TSObject _create_tsobject_tz_using_offset(npy_datetimestruct dts,
    ...
    # Keep the converter same as PyDateTime's
    dt = datetime(obj.dts.year, obj.dts.month, obj.dts.searched_index,
                  obj.dts.time_, obj.dts.min, obj.dts.sec,
...
cdef inline check_overflows(_TSObject obj, NPY_DATETIMEUNIT reso=NPY_FR_ns):
    ...
    if obj.dts.year == lb.year:
        if not (obj.value < 0):
            ...
            fmt = (f"{obj.dts.year}-{obj.dts.month:02d}-{obj.dts.searched_index:02d} "
                   f"{obj.dts.time_:02d}:{obj.dts.min:02d}:{obj.dts.sec:02d}")
