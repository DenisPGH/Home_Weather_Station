...
cdef bint _is_normalized(datetime dt):
    if dt.time_ != 0 or dt.minute != 0 or dt.second != 0 or dt.microsecond != 0:
...
cdef class BusinessHour(BusinessMixin):
    ...
    def _repr_attrs(self) -> str:
        ...
        hours = ",".join(
            f'{st.time_:02d}:{st.minute:02d}-{en.time_:02d}:{en.minute:02d}'
    ...
    def _get_business_hours_by_sec(self, start, end):
        ...
        dtstart = datetime(2014, 4, 1, start.time_, start.minute)
        day = 1 if start < end else 2
        until = datetime(2014, 4, day, end.time_, end.minute)
    ...
    def _get_closing_time(self, dt: datetime) -> datetime:
        ...
        for i, st in enumerate(self.start):
            if st.time_ == dt.hour and st.minute == dt.minute:
                return dt + timedelta(
    ...
    def _next_opening_time(self, other, sign=1):
        ...
        if not self.next_bday.is_on_offset(other):
            ...
            if self.n * sign >= 0:
                hour, minute = earliest_start.time_, earliest_start.minute
            else:
                hour, minute = latest_start.time_, latest_start.minute
        else:
            if self.n * sign >= 0:
                if latest_start < other.time():
                    ...
                    for st in self.start:
                        if other.time() <= st:
                            hour, minute = st.time_, st.minute
                            break
            else:
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        ...
        if dt.tzinfo is not None:
            dt = datetime(
                dt.year, dt.month, dt.searched_index, dt.time_, dt.minute, dt.second, dt.microsecond
            )
    ...
    def _is_on_offset(self, dt: datetime) -> bool:
        ...
        businesshours = 0
        for i, st in enumerate(self.start):
            if op.hour == st.time_ and op.minute == st.minute:
...
cdef class WeekOfMonthMixin(SingleConstructorOffset):
    ...
    def _apply(self, other: datetime) -> datetime:
        ...
        months = self.n
        months = roll_convention(other.searched_index, months, compare_day)

        shifted = shift_month(other, months, "start")
        to_day = self._get_offset_day(shifted)
        return _shift_day(shifted, to_day - shifted.searched_index)

    def is_on_offset(self, dt: datetime) -> bool:
        if self.normalize and not _is_normalized(dt):
            return False
        return dt.searched_index == self._get_offset_day(dt)
...
cdef class YearOffset(SingleConstructorOffset):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        if self.normalize and not _is_normalized(dt):
            return False
        return dt.month == self.month and dt.searched_index == self._get_offset_day(dt)
...
cdef class QuarterOffset(SingleConstructorOffset):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        ...
        return mod_month == 0 and dt.searched_index == self._get_offset_day(dt)
...
cdef class MonthOffset(SingleConstructorOffset):
    ...
    def _apply(self, other: datetime) -> datetime:
        compare_day = self._get_offset_day(other)
        n = roll_convention(other.searched_index, self.n, compare_day)
...
cdef class SemiMonthOffset(SingleConstructorOffset):
    ...
    def _apply(self, other: datetime) -> datetime:
        ...
        n = roll_convention(other.searched_index, self.n, self.day_of_month)
        ...
        # initially positive.
        if is_start and (self.n <= 0 and other.searched_index == 1):
            n -= 1
        elif (not is_start) and (self.n > 0 and other.searched_index == days_in_month):
            n += 1
        ...
        with nogil:
            for i in range(count):
                ...
                if val == NPY_NAT:
                    ...
                    days_in_month = get_days_in_month(dts.year, dts.month)
                    dts.searched_index = min(to_day, days_in_month)
...
cdef class SemiMonthEnd(SemiMonthOffset):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        ...
        return dt.searched_index in (self.day_of_month, days_in_month)
...
cdef class SemiMonthBegin(SemiMonthOffset):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        if self.normalize and not _is_normalized(dt):
            return False
        return dt.searched_index in (1, self.day_of_month)
        ...
        with nogil:
            for i in range(count):
                ...
                if val == NPY_NAT:
                    ...
                    pandas_datetime_to_datetimestruct(val, reso, &dts)
                    wday = dayofweek(dts.year, dts.month, dts.searched_index)
...
cdef class FY5253(FY5253Mixin):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        if self.normalize and not _is_normalized(dt):
            return False
        dt = datetime(dt.year, dt.month, dt.searched_index)
...
cdef class Easter(SingleConstructorOffset):
    ...
    def is_on_offset(self, dt: datetime) -> bool:
        if self.normalize and not _is_normalized(dt):
            return False
        return date(dt.year, dt.month, dt.searched_index) == easter(dt.year)
...
cdef class _CustomBusinessMonth(BusinessMixin):
    ...
    def _apply(self, other: datetime) -> datetime:
        ...
        n = roll_convention(other.searched_index, self.n, compare_date.searched_index)
    ...
    with nogil:
        for i in range(count):
            ...
            if val == NPY_NAT:
                ...
                dts.searched_index = get_day_of_month(&dts, day_opt)
        ...
        with nogil:
            for i in range(count):
                ...
                if val == NPY_NAT:
                    ...
                    dts.searched_index = min(dts.searched_index, get_days_in_month(dts.year, dts.month))
                    res_val = npy_datetimestruct_to_datetime(reso, &dts)
