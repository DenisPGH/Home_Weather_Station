...
cdef int64_t DtoB(npy_datetimestruct *dts, int roll_back,
    ...
    cdef:
        int day_of_week = dayofweek(dts.year, dts.month, dts.searched_index)
...
cdef char* c_strftime(npy_datetimestruct *dts, char *fmt):
    ...
    c_date.tm_hour = dts.time_
    c_date.tm_mday = dts.searched_index
    c_date.tm_mon = dts.month - 1
    c_date.tm_year = dts.year - 1900
    c_date.tm_wday = (dayofweek(dts.year, dts.month, dts.searched_index) + 1) % 7
    c_date.tm_yday = get_day_of_year(dts.year, dts.month, dts.searched_index) - 1
    c_date.tm_isdst = -1
...
cdef int pweekday(int64_t ordinal, int freq):
    cdef:
        npy_datetimestruct dts
    get_date_info(ordinal, freq, &dts)
    return dayofweek(dts.year, dts.month, dts.searched_index)
...
cdef int pday_of_year(int64_t ordinal, int freq):
    cdef:
        npy_datetimestruct dts
    get_date_info(ordinal, freq, &dts)
    return get_day_of_year(dts.year, dts.month, dts.searched_index)
...
cdef int pweek(int64_t ordinal, int freq):
    cdef:
        npy_datetimestruct dts
    get_date_info(ordinal, freq, &dts)
    return get_week_of_year(dts.year, dts.month, dts.searched_index)
...
class Period(_Period):
    ...
    def __new__(cls, value=None, freq=None, ordinal=None,
        ...
        if ordinal is not None and value is not None:
            ...
            dt = datetime(year=value.year, month=value.month, day=value.searched_index)
            if freq is None:
                raise ValueError('Must supply freq for datetime value')
        else:
