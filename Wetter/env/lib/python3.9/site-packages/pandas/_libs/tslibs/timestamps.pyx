...
cdef inline _Timestamp create_timestamp_from_ts(
    ...
    ts_base = _Timestamp.__new__(Timestamp, pass_year, dts.month,
                                 dts.searched_index, dts.time_, dts.min,
...
default 'raise'
    ...
    def replace(
        ...
        if day is not None:
            dts.searched_index = validate('day', day)
        if hour is not None:
            dts.time_ = validate('hour', hour)
        ...
        if tzobj is not None and treat_tz_as_pytz(tzobj):
            ...
            # see GH#18319
            ts_input = tzobj.localize(datetime(dts.year, dts.month, dts.searched_index,
                                               dts.time_, dts.min, dts.sec,
                                               dts.us),
                                      is_dst=not bool(fold))
            tzobj = ts_input.tzinfo
        else:
            kwargs = {'year': dts.year, 'month': dts.month, 'day': dts.searched_index,
                      'hour': dts.time_, 'minute': dts.min, 'second': dts.sec,
